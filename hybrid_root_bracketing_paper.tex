...
In this work, we introduce four new hybrid root bracketing algorithms that combine traditional bracketing methods with optimization strategies. These algorithms are designed to maintain guaranteed convergence properties while improving convergence rates through intelligent interval reduction and the incorporation of modified secant techniques for enhanced performance. The proposed methods are constructed to leverage the strengths of both bracketing and open methods, ensuring that the reliability of the former is preserved while the speed of the latter is harnessed.
...
\begin{table}[H]
\centering
\caption{Detailed Iteration Counts for Each Test Problem. This table presents the number of iterations required by each algorithm to converge for every test problem, highlighting the consistency and efficiency of the proposed methods across diverse function types.}
...
In conclusion, this paper has presented four novel hybrid root bracketing algorithms that successfully combine traditional numerical methods with optimization techniques. The results show that all four hybrid algorithms achieve significantly faster convergence than traditional methods, with Optimized\_BFMS demonstrating the best overall performance. The algorithms maintain a 100\% success rate across all test functions, and the average iteration count is reduced by over 90\% compared to classical approaches. These findings highlight the effectiveness of optimization-based hybrid methods in numerical root finding and suggest that such approaches can be immediately applied to a wide range of scientific and engineering problems.

The theoretical contributions of this work include the development of four new hybrid algorithms with solid mathematical foundations, each combining different aspects of traditional bracketing methods with optimization-based enhancements. The Optimized\_BF algorithm successfully merges bisection and false position methods, while Optimized\_BFMS adds a modified secant step for further acceleration. The trisection-based variants (Optimized\_TF and Optimized\_TFMS) demonstrate the effectiveness of three-way interval splitting combined with false position and secant techniques. All algorithms maintain the guaranteed convergence properties of bracketing methods while achieving significantly improved convergence rates.

The experimental contributions include a comprehensive performance analysis on a diverse set of 14 test functions covering transcendental, polynomial, trigonometric, logarithmic, and exponential categories. The rigorous experimental framework, utilizing SymPy for symbolic computation and SQLite for data management, provided statistically significant results through 1000-fold repetition of each algorithm execution. The high-precision tolerance of $10^{-14}$ ensured accurate root approximations, while the comprehensive performance metrics allowed for detailed comparative analysis.

The practical implications of this work are significant for the field of numerical analysis. The proposed algorithms can be immediately applied to various scientific and engineering problems requiring efficient root finding capabilities. The implementation using SymPy provides a robust foundation for further development and adaptation to specific application domains. The database-driven experimental framework facilitates future comparative studies and performance analysis.
